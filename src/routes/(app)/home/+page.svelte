<script lang="ts">
	...
	const calculateRecommendations = () => {
		...
		const subsidy =
			selectedLocation === 'Rural' ? 30000 : selectedLocation === 'Suburban' ? 20000 : 10000;
...
<div class="flex min-h-screen flex-col items-center bg-gray-50 px-4">
	<!-- Header -->
	<section class="w-full bg-yellow-300 px-6 py-16 text-center shadow-lg">
		<h1 class="mb-4 text-5xl font-extrabold text-yellow-800">Solar Energy Planner</h1>
		<p class="mx-auto max-w-3xl text-lg font-medium text-gray-700">
			Customize your energy plan with SolarVerve! Let us help you make the switch to renewable
			energy.
		</p>
	</section>

	<!-- Form Section -->
	<section class="mt-12 w-full max-w-4xl rounded-2xl bg-white px-8 py-12 shadow-lg">
		...
		<form on:submit|preventDefault={calculateRecommendations} class="space-y-8">
			<!-- Location Input -->
			<div>
				<label for="location" class="mb-2 block text-lg font-medium text-gray-700"
					>Your Location</label
				>
				<select
					id="location"
			...
			<!-- Energy Need Input -->
			<div>
				<label for="energy-need" class="mb-2 block text-lg font-medium text-gray-700"
					>Energy Needs</label
				>
				<select
					id="energy-need"
			...
			<!-- Budget Input -->
			<div>
				<label for="budget" class="mb-2 block text-lg font-medium text-gray-700">Budget Range</label
				>
				<select
					id="budget"
			...
			<button
				type="submit"
				class="flex w-full items-center justify-center rounded-full bg-yellow-500 px-6 py-3 text-lg font-bold text-white shadow-lg hover:bg-yellow-600 hover:shadow-xl"
			>
				<span>Calculate</span>
	...
	<!-- Results Section -->
	{#if result}
		<section class="mt-8 w-full max-w-4xl rounded-2xl bg-yellow-50 px-8 py-6 shadow-md">
			<h3 class="mb-4 text-2xl font-bold text-yellow-700">Your Results</h3>
			<p class="whitespace-pre-line leading-7 text-gray-700">{result}</p>
		</section>
	{/if}
</div>
